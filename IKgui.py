...
def canvas_drag(event):
    ...
    arms.solve([x, y], fixed = fixed.get())                      # solve and render IK
    arms.update()
...
e_lengths.grid(row=2, column=0, sticky = W+E, columnspan=8)
e_lengths.insert(0, txtLen)
...
c_pattern = Checkbutton(master, text="Anchored", bg="black", fg ="white", activebackground = "black", activeforeground = "gray", selectcolor="black", variable=fixed, relief = FLAT, borderwidth=0)
fixed.set(True)
c_pattern.grid(row=2, column=8, sticky = E)
